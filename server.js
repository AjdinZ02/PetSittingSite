...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    if (!fullname || !username || !password || password.length < 6) {
      return res.status(400).json({ error: 'Unesite ime i prezime, korisničko ime i lozinku (min 6 znakova).' });
    }
    const exists = await findUserByUsername(username);
    if (exists) return res.status(409).json({ error: 'Korisničko ime je zauzeto.' });
    const user = await createUser({ fullname, username, password, email, phone, address, role: 'user' });
